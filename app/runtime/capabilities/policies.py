...
class CapabilityPolicy(BaseModel):
    ...
    idempotent: bool = Field(
        default=False, description="Whether the operation is safe to retry blindly."
    )
    rate_limit_per_minute: int | None = Field(
        default=None, description="Maximum executions per minute."
    )
    requires_audit: bool = Field(
        default=False, description="Whether the execution requires audit logging."
    )
