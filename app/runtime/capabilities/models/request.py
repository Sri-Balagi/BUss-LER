...
class CapabilityRequest(BaseModel):
    ...
    execution_scope: str | None = Field(default=None, description="Scope of the execution (e.g., global, tenant).")
    permissions: list[CapabilityPermission] = Field(default_factory=list, description="Permissions granted for this request.")
    timeout_ms: int | None = Field(default=None, description="Explicit timeout for this specific request.")
    trace_id: UUID | None = Field(default=None, description="Trace ID for observability.")
    caller_id: str | None = Field(default=None, description="Identity of the caller (e.g., agent ID).")
