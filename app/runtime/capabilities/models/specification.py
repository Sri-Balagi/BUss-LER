...
class CapabilitySpecification(BaseModel):
    ...
    supported_operations: list[str] = Field(
        default_factory=list, description="Operations this capability can perform."
    )
    permissions_required: list[CapabilityPermission] = Field(
        default_factory=list, description="Permissions required to execute."
    )
    ...
    policy: CapabilityPolicy = Field(
        default_factory=CapabilityPolicy, description="Execution constraints."
    )
