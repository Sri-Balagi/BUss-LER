"""Intent Repository — Supabase implementation.

Table: intents
"""

import time
from abc import ABC, abstractmethod
from typing import Any
from uuid import UUID

import structlog
from postgrest.types import CountMethod
from supabase import AsyncClient

from app.intelligence.intake.intent.intent import (
    Intent,
    IntentCreate,
    IntentStatus,
    IntentType,
    IntentUpdate,
    PaginatedIntents,
)
from app.shared.exceptions.errors import IntentNotFoundError, RepositoryError

logger = structlog.get_logger(__name__)


class AbstractIntentRepository(ABC):
    @abstractmethod
    async def create(self, twin_id: UUID, data: IntentCreate) -> Intent:
        pass

    @abstractmethod
    async def get_by_id(self, intent_id: UUID) -> Intent:
        pass

    @abstractmethod
    async def list_by_twin(
        self,
        twin_id: UUID,
        status: IntentStatus | None = None,
        intent_type: IntentType | None = None,
        limit: int = 20,
        offset: int = 0,
        include_deleted: bool = False,
    ) -> PaginatedIntents:
        pass

    @abstractmethod
    async def update(self, intent_id: UUID, data: IntentUpdate) -> Intent:
        pass

    @abstractmethod
    async def soft_delete(self, intent_id: UUID) -> None:
        pass

    @abstractmethod
    async def health_check(self) -> dict:
        pass


class IntentRepository(AbstractIntentRepository):
    """Data access layer for the ``intents`` table."""

    _table_name = "intents"

    def __init__(self, client: AsyncClient) -> None:
        self._client = client

    async def create(self, twin_id: UUID, data: IntentCreate) -> Intent:
        start = time.time()
        insert_data: dict[str, Any] = {
            "twin_id": str(twin_id),
            "raw_text": data.raw_text,
            "intent_type": data.intent_type.value,
            "status": data.status.value,
        }
        if data.title:
            insert_data["title"] = data.title
        if data.analysis:
            insert_data["analysis"] = data.analysis.model_dump()
        if data.metadata:
            insert_data["metadata"] = data.metadata

        try:
            response = await self._client.table(self._table_name).insert(insert_data).execute()
        except Exception as exc:
            logger.error("Failed to create intent", error=str(exc))
            raise RepositoryError("intent.create", str(exc)) from exc

        duration_ms = (time.time() - start) * 1000
        first_row = response.data[0] if response.data and isinstance(response.data, list) else {}
        intent_id = first_row.get("id") if isinstance(first_row, dict) else ""
        logger.info("Created intent", intent_id=str(intent_id), latency_ms=duration_ms)
        return self._deserialize(first_row)

    async def get_by_id(self, intent_id: UUID) -> Intent:
        start = time.time()
        try:
            response = (
                await self._client.table(self._table_name)
                .select("*")
                .eq("id", str(intent_id))
                .execute()
            )
        except Exception as exc:
            logger.error("Failed to fetch intent", intent_id=str(intent_id), error=str(exc))
            raise RepositoryError("intent.get_by_id", str(exc)) from exc

        if not response.data:
            raise IntentNotFoundError(str(intent_id))

        duration_ms = (time.time() - start) * 1000
        logger.debug("Fetched intent", intent_id=str(intent_id), latency_ms=duration_ms)
        return self._deserialize(response.data[0])

    async def list_by_twin(
        self,
        twin_id: UUID,
        status: IntentStatus | None = None,
        intent_type: IntentType | None = None,
        limit: int = 20,
        offset: int = 0,
        include_deleted: bool = False,
    ) -> PaginatedIntents:
        start = time.time()
        try:
            query = (
                self._client.table(self._table_name)
                .select("*", count=CountMethod.exact)
                .eq("twin_id", str(twin_id))
            )
            if not include_deleted:
                query = query.is_("deleted_at", "null")
            if status:
                query = query.eq("status", status.value)
            if intent_type:
                query = query.eq("intent_type", intent_type.value)

            response = (
                await query.order("created_at", desc=True)
                .range(offset, offset + limit - 1)
                .execute()
            )
        except Exception as exc:
            logger.error("Failed to list intents", twin_id=str(twin_id), error=str(exc))
            raise RepositoryError("intent.list_by_twin", str(exc)) from exc

        items = [self._deserialize(row) for row in response.data]
        total = response.count if response.count is not None else len(items)
        duration_ms = (time.time() - start) * 1000
        logger.debug(
            "Listed intents",
            twin_id=str(twin_id),
            count=len(items),
            total=total,
            latency_ms=duration_ms,
        )
        return PaginatedIntents(items=items, total_count=total, limit=limit, offset=offset)

    async def update(self, intent_id: UUID, data: IntentUpdate) -> Intent:
        start = time.time()
        update_data: dict[str, Any] = {}
        if data.title is not None:
            update_data["title"] = data.title
        if data.intent_type is not None:
            update_data["intent_type"] = data.intent_type.value
        if data.status is not None:
            update_data["status"] = data.status.value
        if data.analysis is not None:
            update_data["analysis"] = data.analysis.model_dump()
        if data.metadata is not None:
            update_data["metadata"] = data.metadata

        if not update_data:
            return await self.get_by_id(intent_id)

        try:
            response = (
                await self._client.table(self._table_name)
                .update(update_data)
                .eq("id", str(intent_id))
                .execute()
            )
        except Exception as exc:
            logger.error("Failed to update intent", intent_id=str(intent_id), error=str(exc))
            raise RepositoryError("intent.update", str(exc)) from exc

        if not response.data:
            raise IntentNotFoundError(str(intent_id))

        duration_ms = (time.time() - start) * 1000
        logger.info("Updated intent", intent_id=str(intent_id), latency_ms=duration_ms)
        first_row = response.data[0] if response.data and isinstance(response.data, list) else {}
        return self._deserialize(first_row)

    async def soft_delete(self, intent_id: UUID) -> None:
        start = time.time()
        now_iso = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        try:
            response = (
                await self._client.table(self._table_name)
                .update({"deleted_at": now_iso, "status": IntentStatus.REJECTED.value})
                .eq("id", str(intent_id))
                .execute()
            )
        except Exception as exc:
            logger.error("Failed to soft delete intent", intent_id=str(intent_id), error=str(exc))
            raise RepositoryError("intent.soft_delete", str(exc)) from exc

        if not response.data:
            raise IntentNotFoundError(str(intent_id))

        duration_ms = (time.time() - start) * 1000
        logger.info("Soft deleted intent", intent_id=str(intent_id), latency_ms=duration_ms)

    async def health_check(self) -> dict:
        status = {"status": "unhealthy", "database": False, "table": False}
        try:
            await self._client.table(self._table_name).select("id").limit(1).execute()
            status.update({"status": "healthy", "database": True, "table": True})
        except Exception as exc:
            logger.error("Intent repository health check failed", error=str(exc))
            status["detail"] = str(exc)
        return status

    @staticmethod
    def _deserialize(row: Any) -> Intent:
        """Map raw Supabase row dict to Intent domain model."""
        from app.intelligence.intake.intent.intent import IntentAnalysis

        row_dict = dict(row) if isinstance(row, dict) else {}
        # Deserialize nested JSONB analysis
        if row_dict.get("analysis") and isinstance(row_dict["analysis"], dict):
            row_dict["analysis"] = IntentAnalysis.model_validate(row_dict["analysis"])
        return Intent.model_validate(row_dict)
