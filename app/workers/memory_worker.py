...
class MemoryProcessingWorker:
    ...
    async def handle_event(self, event: MemoryLifecycleEvent) -> None:
        ...
        if event.event_type != EventType.CREATED:
            logger.debug(
                "Skipping unhandled event type", event_type=event.event_type.value
            )
            return
        ...
        ctx = OperationContext(
            correlation_id=event.correlation_id, twin_id=event.twin_id
        )
        ...
        try:
            ...
            try:
                ...
                new_state = MemoryStateMachine.transition(
                    event.memory_id, memory.embedding_status, EmbeddingStatus.FAILED
                )
                await self._memory_service.update_embedding_status(
                    ctx, event.memory_id, new_state
                )
    ...
    async def _process_memory_pipeline(
        self,
        ...
        new_state = MemoryStateMachine.transition(
            event.memory_id, memory.embedding_status, EmbeddingStatus.PROCESSING
        )
        await self._memory_service.update_embedding_status(
            ctx, event.memory_id, new_state
        )
        ...
        # 6. Mark as COMPLETED
        final_state = MemoryStateMachine.transition(
            memory.id, new_state, EmbeddingStatus.COMPLETED
        )
