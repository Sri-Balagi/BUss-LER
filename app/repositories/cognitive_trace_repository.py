...
from app.models.cognitive_trace import CognitiveTrace, CognitiveTraceCreate, PaginatedCognitiveTraces
...
class CognitiveTraceRepository(AbstractCognitiveTraceRepository):
    ...
    async def list_by_operation(self, operation_type: str, limit: int = 20, offset: int = 0) -> PaginatedCognitiveTraces:
        try:
            response = (
                await self._client.table(self._table)
                .select("*", count="exact")
            ...
            raise RepositoryError("cognitive_trace.list_by_operation", str(exc)) from exc

        items = [self._deserialize(row) for row in response.data]
        total = response.count if response.count is not None else len(items)
        return PaginatedCognitiveTraces(items=items, total_count=total, limit=limit, offset=offset)

    async def list_by_context(self, operation_context_id: str, limit: int = 20, offset: int = 0) -> PaginatedCognitiveTraces:
        try:
            response = (
                await self._client.table(self._table)
                .select("*", count="exact")
    ...
    def _deserialize(row: dict) -> CognitiveTrace:
        ...
        from app.models.cognitive_trace import CognitiveTraceTokenUsage
