...
class CreateMemoryUseCase:
    ...
    async def execute(self, twin_id: uuid.UUID, data: MemoryCreate, correlation_id: str) -> Memory:
        logger.info(
            "Starting memory creation orchestration",
            twin_id=str(twin_id),
        ...
        if not memory.summary and len(memory.content) > 100:
            try:
                ...
                logger.warning(
                    "Failed to generate memory summary", memory_id=str(memory.id), error=str(e)
                )
        ...
        try:
            ...
            logger.error(
                "Failed to generate/upsert embedding for memory",
