...
class AbstractMemoryService:
    ...
    async def create_memory(
        self, ctx: OperationContext, cmd: CreateMemoryCommand
    ) -> CreateMemoryResult:
        pass

    async def delete_memory(
        self, ctx: OperationContext, cmd: DeleteMemoryCommand
    ) -> DeleteMemoryResult:
        pass

    async def restore_memory(
        self, ctx: OperationContext, cmd: RestoreMemoryCommand
    ) -> None:
        pass

    async def update_summary(
        self, ctx: OperationContext, memory_id: UUID, summary: str
    ) -> Memory:
        pass

    async def update_embedding_status(
        self, ctx: OperationContext, memory_id: UUID, status: EmbeddingStatus
    ) -> Memory:
        pass
    ...
    async def search_memories(
        self, ctx: OperationContext, query: MemorySearchQuery
    ) -> SearchMemoryResult:
        pass
...
class MemoryService(AbstractMemoryService):
    ...
    async def restore_memory(
        ...
        try:
            ...
            raise ServiceError(
                f"Memory restoration orchestration failed: {str(e)}"
            ) from e
    ...
    async def update_embedding_status(
        ...
        try:
            ...
            latency = (time.time() - start_time) * 1000
            log.info(
                "Updated embedding status", status=status.value, latency_ms=latency
            )
            return memory
        except RepositoryError as e:
    ...
    async def list_memories(
        ...
        try:
            ...
            result = await self._metadata_repo.list_by_twin(
                twin_id, limit, offset, include_deleted
            )
            ...
            if category:
                ...
                result = PaginatedMemories(
                    items=items, total_count=len(items), limit=limit, offset=offset
                )
    ...
    async def search_memories(
        ...
        try:
            ...
            # 1. Embed query text
            embed_response = await self._ai_kernel.embed(
                EmbeddingRequest(text=query.query_text)
            )
            ...
            for vec_res in vector_results:
                ...
                try:
                    ...
                    results.append(
                        MemorySearchResultItem(
                            memory=memory, similarity_score=vec_res.score
                        )
                    )
                except RepositoryError:
                    log.warning(
                        "Vector found but metadata missing or deleted",
                        memory_id=str(vec_res.id),
                    )

            latency = (time.time() - start_time) * 1000
            log.info(
                "Memory search orchestrated", hits=len(results), latency_ms=latency
            )
    ...
    async def check_health(self) -> dict:
        ...
        try:
            ...
            health_status["subsystems"]["metadata_repository"] = (
                f"healthy ({latency:.1f}ms)"
            )
        except Exception as e:
        ...
        try:
            ...
            await self._vector_repo.search(
                query_vector=[0.0] * 768, twin_id=uuid.uuid4(), limit=1
            )
            latency = (time.time() - start) * 1000
            health_status["subsystems"]["vector_repository"] = (
                f"healthy ({latency:.1f}ms)"
            )
        except Exception as e:
            health_status["status"] = "degraded"
