...
from app.connectors.builtin.calendar.gcalendar.resources import (
...
async def _build_calendar_service(
    ...
    access_token = await manager.get_live_token("google", tenant_id, account_id)
    if not access_token:
        raise RuntimeError(
            "Google credentials not found. Authenticate via OAuth callback."
        )
...
class GoogleCalendarConnector(BaseConnector):
    ...
    async def authenticate(
        ...
        from app.connectors.oauth.manager import OAuthProviderManager
        manager = OAuthProviderManager()
        url = manager.get_auth_url("google", client_id=os.environ.get("GOOGLE_CLIENT_ID", ""), redirect_uri="http://localhost:8000/api/v1/oauth/callback", state=f"{tenant_id}:{account_id}")
        return {"auth_url": url}
    ...
    async def search(
        ...
        svc = await _build_calendar_service(context.tenant_id)
    ...
    async def execute_action(
        ...
        cap = action
        p = params
        svc = await _build_calendar_service(context.tenant_id, "default")
        ...
        if cap == "list_calendars":
            ...
            return await execute_with_resilience(
                ...
                lambda: res2.list_events(
                    ...
                    query=p.get("query"),
                    max_results=p.get("page_size", 50),
