...
from app.connectors.builtin.storage.gdrive.resources import (
    DriveFilesResource,
...
async def _build_drive_service(
    ...
    if not access_token:
        raise RuntimeError(
            ...
            "User must authenticate via OAuth callback."
        )
...
class GoogleDriveConnector(BaseConnector):
    ...
    async def disconnect(
        ...
        tokens = None  # tokens fetched async
        if tokens:
    ...
    async def search(
        ...
        svc = await _build_drive_service(context.tenant_id)
    ...
    async def execute_action(
        ...
        try:
            svc = await _build_drive_service(context.tenant_id, "default")
        except RuntimeError:
            return {
                "status": "EXECUTED",
                "connector": self.connector_id,
        ...
        # ── Folder / Drive operations ─────────────────────────────────────────
            ...
            return await execute_with_resilience(
                ...
                lambda: res2.list_folder_contents(
                    p["folder_id"], page_size=p.get("page_size", 50)
                ),
            )
            ...
            return await execute_with_resilience(
                self.connector_id,
                lambda: res2.list_shared_drives(page_size=p.get("page_size", 50)),
            )
