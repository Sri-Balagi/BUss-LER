...
def test_situation_analysis_healthy():
    ...
    kpis = [KPIAssessment(kpi_id="rev", current_value=100, target_value=100, status=KPIStatus.HEALTHY, deviation_percentage=0.0)]
...
def test_situation_analysis_risks_and_opportunities():
    ...
    kpis = [
        KPIAssessment(kpi_id="rev", current_value=70, target_value=100, status=KPIStatus.CRITICAL, deviation_percentage=-30.0),
        KPIAssessment(kpi_id="traffic", current_value=90, target_value=100, status=KPIStatus.WARNING, deviation_percentage=-10.0)
    ]
    world_model = BusinessWorldModel()
    world_model.update_belief(Belief("customer_sentiment", "improving", 0.9))
    
    intent = ExecutiveIntent(raw_request="Grow", classification=IntentClassification.STRATEGIC_OBJECTIVE)
