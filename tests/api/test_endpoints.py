...
def test_create_entity_success(client, mock_entity_service):
    user_id = uuid4()
    entity_id = uuid4()
    mock_entity = Entity(id=entity_id, user_id=user_id, name="Test API Entity", entity_type=EntityType.STARTUP, description=None, metadata={}, is_active=True, created_at="2026-06-25T00:00:00Z", updated_at="2026-06-25T00:00:00Z")
    mock_entity_service.create_entity.return_value = mock_entity
    
    response = client.post("/api/v1/entities", json={"name": "Test API Entity", "entity_type": "startup"})
...
def test_create_entity_malformed_json(client):
    response = client.post("/api/v1/entities", data="NOT JSON", headers={"Content-Type": "application/json"})
    assert response.status_code == 422 # FastAPI returns 422 for unparseable JSON in newer versions, or 400.
    
def test_create_entity_invalid_enum(client):
    response = client.post("/api/v1/entities", json={"name": "Test API Entity", "entity_type": "INVALID_ENUM"})
    assert response.status_code == 422

def test_get_entity_success(client, mock_entity_service):
    entity_id = uuid4()
    mock_entity = Entity(id=entity_id, user_id=uuid4(), name="Test API Entity", entity_type=EntityType.STARTUP, description=None, metadata={}, is_active=True, created_at="2026-06-25T00:00:00Z", updated_at="2026-06-25T00:00:00Z")
...
def test_create_twin_success(client, mock_twin_service):
    entity_id = uuid4()
    twin_id = uuid4()
    mock_twin = DigitalTwin(id=twin_id, entity_id=entity_id, state={"k": "v"}, metadata=TwinMetadata(), twin_version=1, last_snapshot_at=None, created_at="2026-06-25T00:00:00Z", updated_at="2026-06-25T00:00:00Z")
...
def test_list_twins_success(client, mock_twin_service):
    twin_id = uuid4()
    mock_twin = DigitalTwin(id=twin_id, entity_id=uuid4(), state={}, metadata=TwinMetadata(), twin_version=1, last_snapshot_at=None, created_at="2026-06-25T00:00:00Z", updated_at="2026-06-25T00:00:00Z")
