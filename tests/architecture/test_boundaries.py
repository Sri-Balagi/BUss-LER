...
def test_services_never_import_provider_sdks():
    ...
    for filepath in service_files:
        ...
        imports = get_imports_for_file(filepath)
        for imp in imports:
            assert not imp.startswith("google.genai"), f"Service file {filepath.name} illegally imports provider SDK: {imp}"
            assert not imp.startswith("openai"), f"Service file {filepath.name} illegally imports provider SDK: {imp}"
...
def test_repositories_never_import_ai_kernel():
    ...
    for filepath in repo_files:
        imports = get_imports_for_file(filepath)
        for imp in imports:
            assert not imp.startswith("app.services.ai"), f"Repository file {filepath.name} illegally imports AI layer: {imp}"
...
def test_providers_never_import_repositories():
    ...
    for filepath in provider_files:
        imports = get_imports_for_file(filepath)
        for imp in imports:
            assert not imp.startswith("app.repositories"), f"Provider file {filepath.name} illegally imports repository: {imp}"
