...
async def test_create_failure(repository, mock_supabase_client):
    ...
    data = ContextLifecycleCreate(context_id=uuid.uuid4(), twin_id=uuid.uuid4(), policy_id="test", schema_version="1.0")
...
async def test_update_status_failure(repository, mock_supabase_client):
    ...
    with pytest.raises(RepositoryError) as exc_info:
        await repository.update_status(uuid.uuid4(), ContextLifecycleUpdate(status=ContextStatus.ASSEMBLED))
