...
from app.models.goal import Goal, GoalCreate, GoalUpdate, GoalIntentLink
from app.models.enums import GoalStatus, GoalType
from app.models.exceptions import GoalNotFoundError, RepositoryError
...
async def test_create_failure(repository, mock_supabase_client):
    ...
    with pytest.raises(RepositoryError):
        await repository.create(uuid.uuid4(), GoalCreate(title="Test", goal_type=GoalType.STRATEGIC, priority=5, success_criteria=[]))
...
async def test_update_success(repository, mock_supabase_client):
    ...
    result = await repository.update(goal_id, GoalUpdate(status=GoalStatus.ACTIVE, goal_type=GoalType.STRATEGIC))
