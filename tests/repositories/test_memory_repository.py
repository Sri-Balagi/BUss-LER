...
async def test_update_memory_enums(repo, mock_supabase_client):
    mem_id = uuid4()
    update_data = MemoryUpdate(memory_category=MemoryCategory.REFLECTION, source=MemorySource.DOCUMENT)
...
async def test_repository_errors(repo, mock_supabase_client):
    ...
    with pytest.raises(RepositoryError):
        await repo.create(uuid4(), MemoryCreate(title="t", memory_category=MemoryCategory.EVENT, source=MemorySource.USER_INPUT, content="c", importance=0.5))
