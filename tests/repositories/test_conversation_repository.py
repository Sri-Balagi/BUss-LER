...
from app.models.enums import ConversationStatus, ConversationRole
...
async def test_add_turn_success(repository, mock_supabase_client):
    ...
    insert_execute.return_value = MagicMock(data=[{
        "id": str(turn_id),
...
async def test_add_turn_insert_failure(repository, mock_supabase_client):
    ...
    with pytest.raises(RepositoryError) as exc_info:
        await repository.add_turn(ConversationTurnCreate(thread_id=uuid.uuid4(), role=ConversationRole.USER, content="Hello"))
