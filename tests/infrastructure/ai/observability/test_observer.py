...
@pytest.mark.asyncio
async def test_observer_delegates_properties(
    observer: ProviderObserver, mock_provider: MockLLMProvider
) -> None:
...
@pytest.mark.asyncio
async def test_generate_success_observability(
    observer: ProviderObserver, mock_provider: MockLLMProvider
) -> None:
...
@pytest.mark.asyncio
async def test_generate_error_observability(
    observer: ProviderObserver, mock_provider: MockLLMProvider
) -> None:
...
@pytest.mark.asyncio
async def test_generate_structured_error(
    observer: ProviderObserver, mock_provider: MockLLMProvider
) -> None:
    ...
    failed_log = next(
        event for event in cap_logs if event["event"] == "llm_generate_structured_failed"
    )
...
@pytest.mark.asyncio
async def test_unexpected_error_observability(
    observer: ProviderObserver, mock_provider: MockLLMProvider
) -> None:
