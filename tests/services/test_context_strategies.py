...
def test_ranker_section_ordering(ranker, policy):
    ...
    section_mem = ContextSection(
        section_id=uuid4(), source=ContextSource.MEMORY, priority=ContextPriority.MEDIUM, items=[], token_estimate=0, retrieved_at=datetime.now(timezone.utc)
    )
    section_intent = ContextSection(
        section_id=uuid4(), source=ContextSource.INTENT, priority=ContextPriority.MEDIUM, items=[], token_estimate=0, retrieved_at=datetime.now(timezone.utc)
    )
...
def test_compressor_collapse_background(compressor):
    # 4 background items
    items = [
        create_item(f"bg {i}", priority=ContextPriority.BACKGROUND)
        for i in range(4)
    ]
...
def test_window_builder_critical_first(window_builder):
    ...
    goal_section = ContextSection(
        section_id=uuid4(), source=ContextSource.GOAL, priority=ContextPriority.CRITICAL, items=[item1], token_estimate=10, retrieved_at=datetime.now(timezone.utc)
    )
    mem_section = ContextSection(
        section_id=uuid4(), source=ContextSource.MEMORY, priority=ContextPriority.MEDIUM, items=[item2], token_estimate=10, retrieved_at=datetime.now(timezone.utc)
    )
...
def test_window_builder_section_empty(window_builder):
    ...
    section = ContextSection(
        section_id=uuid4(), source=ContextSource.MEMORY, priority=ContextPriority.MEDIUM, items=[item], token_estimate=100, retrieved_at=datetime.now(timezone.utc)
    )
...
def test_window_builder_zero_budget_skip(window_builder):
    ...
    mem_section1 = ContextSection(
        section_id=uuid4(), source=ContextSource.MEMORY, priority=ContextPriority.MEDIUM, items=[item2], token_estimate=5, retrieved_at=datetime.now(timezone.utc)
    )
