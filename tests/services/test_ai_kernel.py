...
from app.models.ai import AIRequest, EmbeddingRequest
from app.models.exceptions import AIKernelError
...
def mock_provider():
    ...
    provider.health_check.return_value = {"status": "healthy", "provider": "mock_provider"}
    return provider
...
async def test_gemini_integration_summarize(gemini_kernel):
    result = await gemini_kernel.summarize("The user created an AI kernel to route provider requests.")
