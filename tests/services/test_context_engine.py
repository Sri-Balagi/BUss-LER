...
from app.services.context_engine import ContextEngine, ContextAssemblyError, ContextValidationError
from app.models.enterprise_context import (
    EnterpriseContextCreate, ExecutionPlan, ContextSection, ContextWindow,
    ProviderFailureRecord, ContextLifecycleCreate, ContextLifecycleUpdate
)
...
from app.models.events import ContextBuiltEvent, ContextPartiallyBuiltEvent
...
@pytest.fixture
def engine(mock_registry, mock_graph, mock_validator, mock_ranker, mock_compressor, mock_window_builder, mock_repository, mock_event_bus, mock_trace_service):
...
@pytest.mark.asyncio
async def test_build_success(engine, mock_graph, mock_registry, mock_validator, mock_ranker, mock_compressor, mock_window_builder):
    ...
    mock_graph.resolve.return_value = ExecutionPlan(batches=[[ContextSource.MEMORY]], total_providers=1)
...
async def test_build_required_provider_failure(engine, mock_graph, mock_registry):
    ...
    mock_graph.resolve.return_value = ExecutionPlan(batches=[[ContextSource.GOAL]], total_providers=1)
