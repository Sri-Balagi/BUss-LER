...
async def test_entity_repository_create_success(entity_repo, mock_supabase_client):
    user_id = uuid4()
    mock_data = {"id": str(uuid4()), "user_id": str(user_id), "name": "Test", "entity_type": "startup", "metadata": {}, "is_active": True, "description": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"}
...
async def test_entity_repository_create_duplicate(entity_repo, mock_supabase_client):
    user_id = uuid4()
    mock_supabase_client.table.return_value.execute.side_effect = Exception("duplicate key value violates unique constraint")
    data = EntityCreate(name="Test", entity_type=EntityType.STARTUP)
...
async def test_entity_repository_create_db_timeout(entity_repo, mock_supabase_client):
    user_id = uuid4()
    mock_supabase_client.table.return_value.execute.side_effect = TimeoutError("Database connection timed out")
    data = EntityCreate(name="Test", entity_type=EntityType.STARTUP)
...
async def test_entity_repository_get_by_id_success(entity_repo, mock_supabase_client):
    entity_id = uuid4()
    mock_data = {"id": str(entity_id), "user_id": str(uuid4()), "name": "Test", "entity_type": "startup", "metadata": {}, "is_active": True, "description": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"}
    mock_supabase_client.table.return_value.execute.return_value = AsyncMock(data=[mock_data])
    entity = await entity_repo.get_by_id(entity_id)
    assert entity.id == entity_id

@pytest.mark.asyncio
async def test_entity_repository_get_by_id_not_found_empty_results(entity_repo, mock_supabase_client):
    entity_id = uuid4()
...
async def test_entity_repository_get_by_id_invalid_uuid(entity_repo, mock_supabase_client):
    ...
    mock_supabase_client.table.return_value.execute.side_effect = Exception("invalid input syntax for type uuid")
    with pytest.raises(RepositoryError) as exc_info:
...
async def test_entity_repository_update_success(entity_repo, mock_supabase_client):
    entity_id = uuid4()
    mock_data = {"id": str(entity_id), "user_id": str(uuid4()), "name": "Updated", "entity_type": "startup", "metadata": {}, "is_active": True, "description": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"}
    mock_supabase_client.table.return_value.execute.return_value = AsyncMock(data=[mock_data])
...
async def test_entity_repository_delete_success(entity_repo, mock_supabase_client):
    entity_id = uuid4()
    mock_data = {"id": str(entity_id), "user_id": str(uuid4()), "name": "Test", "entity_type": "startup", "metadata": {}, "is_active": False, "description": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"}
    mock_supabase_client.table.return_value.execute.return_value = AsyncMock(data=[mock_data])
...
async def test_entity_repository_list_success(entity_repo, mock_supabase_client):
    user_id = uuid4()
    mock_data = [{"id": str(uuid4()), "user_id": str(user_id), "name": f"Test {i}", "entity_type": "startup", "metadata": {}, "is_active": True, "description": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"} for i in range(5)]
...
async def test_twin_repository_create_success(twin_repo, mock_supabase_client):
    entity_id = uuid4()
    mock_data = {
        "id": str(uuid4()), "entity_id": str(entity_id), "state": {"key": "val"},
        "metadata": {"schema_version": 1, "labels": [], "external_ids": {}, "source": "system", "created_by": "system", "updated_by": "system"}, "twin_version": 1, "last_snapshot_at": None,
...
async def test_twin_repository_get_by_id_success(twin_repo, mock_supabase_client):
    twin_id = uuid4()
    mock_data = {
        "id": str(twin_id), "entity_id": str(uuid4()), "state": {"key": "val"},
        "metadata": {"schema_version": 1, "labels": [], "external_ids": {}, "source": "system", "created_by": "system", "updated_by": "system"}, "twin_version": 1, "last_snapshot_at": None,
        "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"
    }
    mock_supabase_client.table.return_value.execute.return_value = AsyncMock(data=[mock_data])
...
async def test_twin_repository_update_with_snapshot_success(twin_repo, mock_supabase_client):
    twin_id = uuid4()
    mock_data = {
        "id": str(twin_id), "entity_id": str(uuid4()), "state": {"key": "new_val"},
        "metadata": {"schema_version": 1, "labels": [], "external_ids": {}, "source": "system", "created_by": "system", "updated_by": "system"}, "twin_version": 2, "last_snapshot_at": None,
        "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"
    }
...
async def test_twin_repository_delete_success(twin_repo, mock_supabase_client):
    twin_id = uuid4()
    mock_data = {
        "id": str(twin_id), "entity_id": str(uuid4()), "state": {},
        "metadata": {"schema_version": 1, "labels": [], "external_ids": {}, "source": "system", "created_by": "system", "updated_by": "system"}, "twin_version": 1, "last_snapshot_at": None,
        "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"
    }
    mock_supabase_client.table.return_value.execute.return_value = AsyncMock(data=[mock_data])
...
async def test_twin_repository_list_pagination(twin_repo, mock_supabase_client):
    mock_data = [{"id": str(uuid4()), "entity_id": str(uuid4()), "state": {}, "metadata": {"schema_version": 1, "labels": [], "external_ids": {}, "source": "system", "created_by": "system", "updated_by": "system"}, "twin_version": 1, "last_snapshot_at": None, "created_at": "2026-06-25T00:00:00Z", "updated_at": "2026-06-25T00:00:00Z"} for i in range(10)]
    mock_execute = AsyncMock(data=mock_data)
