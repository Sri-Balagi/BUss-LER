...
from app.runtime.retry.backoff import FixedDelay, LinearBackoff, ExponentialBackoff
...
def test_default_retry_strategy():
    ...
    desc_ok = ExecutionDescriptor(execution_type=ExecutionType.SYSTEM, target="mock", metadata={"estimated_cost": 1.0})
    task_ok = Task(id=uuid4(), execution_descriptor=desc_ok, task_priority=TaskPriority.NORMAL)
    
    desc_bad = ExecutionDescriptor(execution_type=ExecutionType.SYSTEM, target="mock", metadata={"estimated_cost": 100.0})
