...
from app.runtime.agents.registry import AgentRegistry
from app.runtime.agents.registry import ResolutionContext as AgentResolutionContext
...
from app.runtime.capabilities.factories import (
    SingletonCapabilityFactory,
    TransientCapabilityFactory,
)
...
# Mocks
from tests.runtime.capabilities.mocks.capabilities import HttpGetCapability, ReadFileCapability
...
class E2EScheduler(AsyncIOScheduler):
    def __init__(
        self, agent_registry: AgentRegistry, capability_manager: CapabilityManager, *args, **kwargs
    ):
    ...
    async def _execute_task_payload(self, task):
        ...
        agent_req = AgentResolutionContext(
            requested_capability=Capability(capability_id=task.descriptor.target, version="1.0")
        )
...
def e2e_env():
    ...
    agent_spec = AgentSpecification(
        identity="cert_agent", version="1.0", capabilities=["cert_agent"], permissions=[]
    )
    ...
    pol_ctx = ExecutionPolicyContext(
        session=session,
