...
def test_agent_manifest_valid():
    manifest = AgentManifest(version="1.0.0", deployment_source="local", author="BizOS Team")
...
def test_agent_result_valid():
    result = AgentResult(
        status=AgentStatus.SUCCESS, outputs={"revenue": 1000}, metrics={"duration_ms": 15.5}
    )
