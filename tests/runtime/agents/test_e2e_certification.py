...
from app.runtime.agents.factories import SingletonFactory, TransientFactory
...
def setup_e2e_environment():
    ...
    policy_ctx = ExecutionPolicyContext(
        session=session, queue_manager=queues, task_dag=dag, budget_manager=budget
    )
    policy = SequentialExecutionPolicy()
    retry = DefaultRetryStrategy(
        default_backoff=ExponentialBackoff(base_delay_ms=10, multiplier=2.0)
    )
...
def test_scenario_1_normal_execution():
    ...
    task = Task(
        execution_descriptor=ExecutionDescriptor(
            execution_type="AGENT", target="math", parameters={"a": 10, "b": 20}
        )
    )
    dag.add_task(task)
...
def test_scenario_2_capability_resolution():
    ...
    task = Task(
        execution_descriptor=ExecutionDescriptor(
            execution_type="AGENT", target="echo", parameters={"message": "hello"}
        )
    )
    queues.get_queue(TaskState.READY).enqueue(task)
    dag.add_task(task)
...
def test_scenario_3_health_degradation_fallback():
    ...
    task = Task(
        execution_descriptor=ExecutionDescriptor(
            execution_type="AGENT", target="unstable", parameters={"message": "survived"}
        )
    )
    queues.get_queue(TaskState.READY).enqueue(task)
    dag.add_task(task)
...
def test_scenario_4_retry_succeeds_after_transient_failure():
    ...
    scheduler, registry, dag, queues, monitor, token, _ = setup_e2e_environment()
    registry.register(
        AgentSpecification(identity="transient", capabilities=["math"]),
        TransientFactory(StatefulAgent),
    )

    task = Task(
        execution_descriptor=ExecutionDescriptor(
            execution_type="AGENT", target="math", parameters={"a": 5, "b": 5}
        )
    )
    queues.get_queue(TaskState.READY).enqueue(task)
    dag.add_task(task)
...
def test_scenario_5_cancellation_propagates():
    ...
    registry.register(
        AgentSpecification(identity="sleeper", capabilities=["sleep"]), TransientFactory(SleepAgent)
    )

    task = Task(
        execution_descriptor=ExecutionDescriptor(
            execution_type="AGENT", target="sleep", parameters={"duration": 1.0}
        )
    )
    queues.get_queue(TaskState.READY).enqueue(task)
    dag.add_task(task)
...
def test_scenario_6_budget_exhaustion():
    ...
    registry.register(
        AgentSpecification(identity="math-1", capabilities=["math"]), TransientFactory(MathAgent)
    )
...
def test_scenario_7_factory_behavior():
    ...
    registry.register(
        AgentSpecification(identity="singleton", capabilities=["count"]),
        SingletonFactory(CounterAgent),
    )
    ...
    counts = {
        t1.descriptor.metadata.get("outputs", {}).get("count"),
        t2.descriptor.metadata.get("outputs", {}).get("count"),
    }
    assert counts == {1, 2}
...
def test_scenario_8_parallel_execution():
    scheduler, registry, dag, queues, monitor, token, _ = setup_e2e_environment()
    registry.register(
        AgentSpecification(identity="math", capabilities=["math"]), TransientFactory(MathAgent)
    )

    for i in range(5):
        t = Task(
            execution_descriptor=ExecutionDescriptor(
                execution_type="AGENT", target="math", parameters={"a": i, "b": i}
            )
        )
...
def test_scenario_9_resolution_trace():
    ...
    registry.register(
        AgentSpecification(identity="echo1", capabilities=["echo"]), TransientFactory(EchoAgent)
    )
    registry.register(
        AgentSpecification(identity="echo2", capabilities=["echo"]), TransientFactory(EchoAgent)
    )
    registry.register(
        AgentSpecification(identity="math1", capabilities=["math"]), TransientFactory(MathAgent)
    )
...
class DummyHook(IAgentHooks):
    ...
    def before_initialize(self, agent):
        self.called.add("before_initialize")

    def after_initialize(self, agent):
        self.called.add("after_initialize")

    def before_execute(self, agent):
        self.called.add("before_execute")

    def after_execute(self, agent, result):
        self.called.add("after_execute")

    def before_suspend(self, agent):
        pass

    def after_resume(self, agent):
        pass

    def before_shutdown(self, agent):
        pass

    def after_shutdown(self, agent):
        pass
...
def test_scenario_10_hooks_and_events():
    ...
    registry.register(
        AgentSpecification(identity="echo", capabilities=["echo"]), TransientFactory(EchoAgent)
    )
