...
class MockHooks(ISchedulerHooks):
    def __init__(self):
        self.log = []
    def before_schedule(self, ctx): self.log.append("before_schedule")
    def before_dispatch(self, task, dec): self.log.append(f"before_dispatch:{task.id}")
    def after_dispatch(self, task): self.log.append(f"after_dispatch:{task.id}")
    def before_retry(self, task, err, ms): self.log.append(f"before_retry:{task.id}")
    def after_retry(self, task): self.log.append(f"after_retry:{task.id}")
    def before_complete(self, task): self.log.append(f"before_complete:{task.id}")
    def after_complete(self, task): self.log.append(f"after_complete:{task.id}")
...
def setup_context():
    ...
    return ExecutionPolicyContext(session=session, queue_manager=QueueManager(), task_dag=TaskDAG(), budget_manager=BudgetManager(budget))

def create_task(target="mock", cost=1.0) -> Task:
    desc = ExecutionDescriptor(execution_type=ExecutionType.SYSTEM, target=target, metadata={"estimated_cost": cost})
...
def test_scheduler_retry_and_failure():
    ...
    assert "Simulated execution failure" in str(t1.execution_descriptor.metadata.get("failure_reason"))
