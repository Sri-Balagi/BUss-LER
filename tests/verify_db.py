...
from app.interfaces.http.schemas.twin import ChangeType, DigitalTwinCreate, DigitalTwinUpdate
...
async def verify():
    ...
    try:
        ...
        try:
            ...
            if (
                "check_violation" in str(e).lower()
        ...
        print(
            f"  OK RPC update_twin_with_snapshot successful. Version incremented to {updated_twin.twin_version}"
        )
        ...
        try:
            ...
            print(
                f"  OK RPC correctly rejected stale version (expected 1, actual {updated_twin.twin_version}): {e}"
            )
        ...
        assert snapshot.snapshot_data["state"]["module"] == "test_updated", (
            "Snapshot state incorrect"
        )
        assert snapshot.snapshot_data["metadata"]["schema_version"] == 1, (
            "Snapshot metadata incorrect"
        )
        ...
        print(
            "  OK Cascading deletes successfully wiped Twin, Snapshot, and History upon Entity hard-delete"
        )
