...
from app.domain.workflows.models import Task, TaskStatus, Workflow
from app.shared.enums import AgentType, ParticipantRole, PrincipalType
...
class InMemorySessionRepository(ISessionRepository):
    ...
    async def get_session(self, session_id: str, tenant_id: str | None) -> Session | None:
...
async def test_normal_workflow_execution(setup_environment):
    ...
    event_bus.publish(ApprovalApprovedEvent(
...
async def test_approval_rejected(setup_environment):
    ...
    event_bus.publish(ApprovalRejectedEvent(
    ...
    await task_repo.get_task(root_task.task_id)
...
async def test_approval_expired(setup_environment):
    ...
    event_bus.publish(ApprovalExpiredEvent(
        correlation_id="corr-2", approval_id="app-1", target_type="task", target_id=executor_task.task_id,
        tenant_id=ctx.tenant_id
    ))
    await event_bus.wait_until_done()
