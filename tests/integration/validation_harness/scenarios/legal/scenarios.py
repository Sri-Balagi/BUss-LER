...
# Generate 40 scenarios for Legal
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Legal End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Legal Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Legal Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="legal",
            scenario_id=f"leg_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
