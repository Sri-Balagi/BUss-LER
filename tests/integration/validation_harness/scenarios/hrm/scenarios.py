...
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Hrm End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Hrm Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Hrm Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="hrm",
            scenario_id=f"hrm_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
