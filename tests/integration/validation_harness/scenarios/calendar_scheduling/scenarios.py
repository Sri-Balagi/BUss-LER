...
# Generate 40 scenarios for Calendar_scheduling
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Calendar Scheduling End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Calendar Scheduling Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Calendar Scheduling Hard Operational Constraint (constraint violations)"
    else:
        desc = f"Execute invalid task {i} for {cat}"
    
    SCENARIOS.append(
        ValidationScenario(
            module_name="calendar_scheduling",
            scenario_id=f"cal_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
