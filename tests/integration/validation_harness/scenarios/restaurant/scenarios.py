...
# Generate 40 scenarios for Restaurant
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Order to Table Service for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Food Safety and Handling Procedures (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Maximum Dining Room Seating Capacity (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"res_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
