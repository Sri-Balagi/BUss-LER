...
# Generate 40 scenarios for Energy
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Energy End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Energy Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Energy Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="energy",
            scenario_id=f"ene_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
