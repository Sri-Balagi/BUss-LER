...
# Generate 40 scenarios for Healthcare
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Emergency Department Patient Triage for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Inpatient Discharge Approval (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Strict Bed Capacity Limit (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"hea_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
