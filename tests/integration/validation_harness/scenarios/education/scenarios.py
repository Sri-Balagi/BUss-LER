...
# Generate 40 scenarios for Education
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Education End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Education Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Education Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"edu_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
