...
# Generate 40 scenarios for Logistics
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Freight Routing and Delivery for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Driver Rest Requirements (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Maximum Vehicle Weight Limit (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"log_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
