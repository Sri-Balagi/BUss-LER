...
# Generate 40 scenarios for Inventory
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Stock Replenishment and Receiving for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing FIFO (First-In, First-Out) Rotation (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Maximum Warehouse Capacity Limit (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"inv_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
