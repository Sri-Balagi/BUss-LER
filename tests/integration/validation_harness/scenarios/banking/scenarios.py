...
# Generate 40 scenarios for Banking
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Mortgage Origination for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Anti-Money Laundering (AML) Checks (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Maximum Debt-to-Income Ratio (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="banking",
            scenario_id=f"ban_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
