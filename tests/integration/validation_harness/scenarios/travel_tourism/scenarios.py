...
# Generate 40 scenarios for Travel_tourism
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Travel Tourism End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Travel Tourism Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Travel Tourism Hard Operational Constraint (constraint violations)"
    else:
        desc = f"Execute invalid task {i} for {cat}"
    
    SCENARIOS.append(
        ValidationScenario(
            module_name="travel_tourism",
            scenario_id=f"tra_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
