...
# Generate 40 scenarios for Airline
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Pre-flight Safety Check Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Airline Safety and Maintenance Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Maximum Takeoff Weight Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="airline",
            scenario_id=f"air_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
