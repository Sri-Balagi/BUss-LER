...
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Lead-to-Cash Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing SLA Response Time Guidelines (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Minimum Deal Size for Enterprise Tier (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            module_name="crm",
            scenario_id=f"crm_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
