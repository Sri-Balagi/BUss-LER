...
# Generate 40 scenarios for Procurement
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Procurement End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Procurement Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Procurement Hard Operational Constraint (constraint violations)"
    else:
