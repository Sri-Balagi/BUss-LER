...
# Generate 40 scenarios for Manufacturing
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Assembly Line Production Run for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Six Sigma Quality Standards (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Machine Operating Temperature Limits (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"man_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
