...
# Generate 40 scenarios for Project_management
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Project Management End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Project Management Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Project Management Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"pro_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
