...
# Generate 40 scenarios for Identity_access_management
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Identity Access Management End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Identity Access Management Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Identity Access Management Hard Operational Constraint (constraint violations)"
    else:
        desc = f"Execute invalid task {i} for {cat}"
    
    SCENARIOS.append(
        ValidationScenario(
            module_name="identity_access_management",
            scenario_id=f"ide_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
