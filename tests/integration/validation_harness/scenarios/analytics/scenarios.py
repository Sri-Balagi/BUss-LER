...
# Generate 40 scenarios for Analytics
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Analytics End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Analytics Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Analytics Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"ana_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
