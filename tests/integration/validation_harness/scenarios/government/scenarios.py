...
# Generate 40 scenarios for Government
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Government End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Government Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Government Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"gov_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
