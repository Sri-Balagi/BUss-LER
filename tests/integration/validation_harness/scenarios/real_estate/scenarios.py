...
# Generate 40 scenarios for Real_estate
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Real Estate End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Real Estate Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Real Estate Hard Operational Constraint (constraint violations)"
    else:
    ...
    SCENARIOS.append(
        ValidationScenario(
            ...
            scenario_id=f"rea_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
