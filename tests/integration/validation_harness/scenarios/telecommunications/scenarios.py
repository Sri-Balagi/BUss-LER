...
# Generate 40 scenarios for Telecommunications
for i in range(1, 41):
    ...
    if cat == "Happy Path":
        desc = f"Execute Telecommunications End-to-End Processing Workflow for task {i}"
    elif cat == "Policy Violations":
        desc = f"Execute invalid task {i} causing Telecommunications Master Regulatory Policy (policy violations)"
    elif cat == "Constraint Violations":
        desc = f"Execute invalid task {i} causing Telecommunications Hard Operational Constraint (constraint violations)"
    else:
        desc = f"Execute invalid task {i} for {cat}"
    
    SCENARIOS.append(
        ValidationScenario(
            module_name="telecommunications",
            scenario_id=f"tel_{cat.replace(' ', '_').lower()}_{i:03d}",
            category=cat,
            description=desc,
