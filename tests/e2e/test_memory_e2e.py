...
async def test_memory_e2e_flow(client):
    ...
    mock_memory_service.create_memory.return_value = CreateMemoryResult(memory=mock_memory, dispatched_events=1)
    ...
    mock_memory_service.delete_memory.return_value = DeleteMemoryResult(success=True, memory_id=str(memory_id))
