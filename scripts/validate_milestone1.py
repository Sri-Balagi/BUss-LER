...
UVICORN_CMD = [sys.executable, "-m", "uvicorn", "app.main:app", "--host", "127.0.0.1", "--port", "8001"]
                ...
                if isinstance(node, ast.ImportFrom):
                    ...
                    if rel_path.startswith("api/") and "dependencies.py" not in rel_path:
                        if "app.repositories" in module:
                            record_fail(f"Architecture violation: Router {rel_path} imports repository ({module})")
                    # Rule: Services must not import Routers
                    if rel_path.startswith("services/"):
                        if "app.api" in module:
                            record_fail(f"Architecture violation: Service {rel_path} imports router ({module})")
                    # Rule: Repositories must not import Services
                    if rel_path.startswith("repositories/"):
                        if "app.services" in module:
                            record_fail(f"Architecture violation: Repository {rel_path} imports service ({module})")
                    # Rule: Models must be dependency-free
                    if rel_path.startswith("models/"):
                        if "app.api" in module or "app.services" in module or "app.repositories" in module:
                            record_fail(f"Architecture violation: Model {rel_path} imports higher layer ({module})")
...
async def validate_security(client: httpx.AsyncClient):
    ...
    # Malformed JSON
    r = await client.post(f"{API_URL}/entities", content="bad json", headers={"Content-Type": "application/json"})
    if r.status_code != 422:
...
async def execute_e2e_workflow(client: httpx.AsyncClient):
    ...
    if r.status_code != 200: record_fail("Health check failed")
    ...
    if r.status_code != 201: record_fail(f"Create Entity failed: {r.text}")
    entity_id = r.json()["id"]
    ...
    if r.status_code != 200: record_fail(f"Retrieve Entity failed: {r.text}")
    ...
    if r.status_code != 200: record_fail(f"Update Entity failed: {r.text}")
    metrics["entity_update"] = f"{(time.time() - t0) * 1000:.2f}ms"
    record_pass("Update Entity passed")
    
    # 5. Create Twin
    t0 = time.time()
    twin_payload = {"entity_id": entity_id, "state": {"stage": "seed"}, "metadata": {"schema_version": 1, "labels": ["test"], "external_ids": {}}}
    r = await client.post(f"{API_URL}/twins", json=twin_payload)
    if r.status_code != 201: record_fail(f"Create Twin failed: {r.text}")
    twin = r.json()
    twin_id = twin["id"]
    if twin["twin_version"] != 1: record_fail("Initial twin version must be 1")
    ...
    if r.status_code != 200: record_fail("Retrieve Twin failed")
    metrics["twin_retrieve"] = f"{(time.time() - t0) * 1000:.2f}ms"
    record_pass("Retrieve Twin passed")
    
    # 7. Update Twin
    t0 = time.time()
    update_payload = {"expected_version": 1, "state": {"stage": "series_a"}, "change_reason": "Raised money"}
    r = await client.put(f"{API_URL}/twins/{twin_id}", json=update_payload)
    if r.status_code != 200: record_fail(f"Update Twin failed: {r.text}")
    if r.json()["twin_version"] != 2: record_fail("Twin version did not increment")
    ...
    if r.status_code != 200 or len(r.json()["items"]) < 1: record_fail("Retrieve Snapshots failed")
    record_pass("Retrieve Snapshots passed")
    
    # 9. Retrieve History
    r = await client.get(f"{API_URL}/twins/{twin_id}/history")
    if r.status_code != 200 or len(r.json()["items"]) < 2: record_fail("Retrieve History failed")
    ...
    if r.status_code != 204: record_fail("Delete Twin failed")
    # Verify cleanup (Twin)
    r = await client.get(f"{API_URL}/twins/{twin_id}")
    if r.status_code != 404: record_fail("Twin was not fully deleted")
    record_pass("Delete Twin passed")
    
    # 12. Delete Entity
    r = await client.delete(f"{API_URL}/entities/{entity_id}")
    if r.status_code != 204: record_fail("Delete Entity failed")
    # Verify cleanup (Entity)
    r = await client.get(f"{API_URL}/entities/{entity_id}")
    if r.status_code != 404: record_fail("Entity was not fully deleted")
...
def generate_reports():
    ...
    try:
        ...
        cov_out = subprocess.run([sys.executable, "-m", "coverage", "report"], capture_output=True, text=True)
