...
def test_validate_transition_success():
    # BUILDING -> ASSEMBLED
    ContextStateMachine.validate_transition(
        ContextStatus.BUILDING, ContextStatus.ASSEMBLED
    )

    # ASSEMBLED -> OPTIMIZED
    ContextStateMachine.validate_transition(
        ContextStatus.ASSEMBLED, ContextStatus.OPTIMIZED
    )

    # OPTIMIZED -> CONSUMED
    ContextStateMachine.validate_transition(
        ContextStatus.OPTIMIZED, ContextStatus.CONSUMED
    )

    # CONSUMED -> ARCHIVED
    ContextStateMachine.validate_transition(
        ContextStatus.CONSUMED, ContextStatus.ARCHIVED
    )
...
def test_validate_transition_failure():
    ...
    with pytest.raises(InvalidStateTransitionError) as exc_info:
        ContextStateMachine.validate_transition(
            ContextStatus.BUILDING, ContextStatus.CONSUMED
        )
    ...
    with pytest.raises(InvalidStateTransitionError):
        ContextStateMachine.validate_transition(
            ContextStatus.ARCHIVED, ContextStatus.BUILDING
        )
