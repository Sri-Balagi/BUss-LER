...
async def test_create_memory_repository_error(memory_service, mock_metadata_repo, ctx):
    ...
    cmd = CreateMemoryCommand(
        twin_id=uuid4(), content="test", memory_category=MemoryCategory.OBSERVATION
    )
...
@pytest.mark.asyncio
async def test_delete_memory_orchestration(
    memory_service, mock_metadata_repo, mock_vector_repo, ctx
):
    mem_id = uuid4()
...
@pytest.mark.asyncio
async def test_search_memories_orchestration(
    memory_service, mock_metadata_repo, mock_vector_repo, mock_ai_kernel, ctx
):
    twin_id = uuid4()
    mem_id = uuid4()
...
@pytest.mark.asyncio
async def test_search_memories_no_ai_kernel(
    mock_metadata_repo, mock_vector_repo, mock_event_bus, ctx
):
...
@pytest.mark.asyncio
async def test_search_memories_empty_vector_results(
    memory_service, mock_ai_kernel, mock_vector_repo, ctx
):
...
@pytest.mark.asyncio
async def test_search_memories_filters(
    memory_service, mock_ai_kernel, mock_vector_repo, mock_metadata_repo, ctx
):
    twin_id = uuid4()
    mock_ai_kernel.embed.return_value = MagicMock(vector=[1, 2])
