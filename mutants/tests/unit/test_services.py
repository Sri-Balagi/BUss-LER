...
@pytest.fixture
def twin_service(
    mock_twin_repo, mock_snapshot_repo, mock_history_repo, mock_entity_repo
):
    return TwinService(
        mock_twin_repo, mock_snapshot_repo, mock_history_repo, mock_entity_repo
    )
...
@pytest.mark.asyncio
async def test_entity_service_create_repository_failure(
    entity_service, mock_entity_repo
):
    user_id = uuid4()
...
@pytest.mark.asyncio
async def test_entity_service_get_by_id_inactive_raises(
    entity_service, mock_entity_repo
):
    entity_id = uuid4()
    mock_entity = Entity(
...
@pytest.mark.asyncio
async def test_twin_service_create_twin_success(
    twin_service,
    mock_twin_repo,
    mock_snapshot_repo,
    mock_history_repo,
...
@pytest.mark.asyncio
async def test_twin_service_create_entity_validation_failure(
    twin_service, mock_entity_repo
):
    entity_id = uuid4()
...
@pytest.mark.asyncio
async def test_twin_service_create_snapshot_failure_rollback(
    twin_service, mock_twin_repo, mock_snapshot_repo, mock_entity_repo
):
    entity_id = uuid4()
    mock_entity_repo.get_by_id.return_value = AsyncMock()
    mock_twin = DigitalTwin(
    ...
    mock_snapshot_repo.create.side_effect = RepositoryError(
        "snapshot.create", "DB Error"
    )
...
@pytest.mark.asyncio
async def test_twin_service_create_history_failure_rollback(
    twin_service,
    mock_twin_repo,
    mock_snapshot_repo,
    mock_history_repo,
...
@pytest.mark.asyncio
async def test_twin_service_update_optimistic_concurrency_failure(
    twin_service, mock_twin_repo
):
    twin_id = uuid4()
