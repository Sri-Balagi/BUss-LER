...
async def test_worker_pipeline_success_no_summary(worker, memory_service, ai_kernel, vector_repo):
    ...
    memory_service.update_embedding_status.assert_any_call(
        ANY, memory_id, EmbeddingStatus.PROCESSING
    )
    memory_service.update_embedding_status.assert_any_call(
        ANY, memory_id, EmbeddingStatus.COMPLETED
    )
...
async def test_worker_retries_and_fails(worker, memory_service, ai_kernel):
    ...
    memory_service.update_embedding_status.assert_called_once_with(
        ANY, memory_id, EmbeddingStatus.FAILED
    )
