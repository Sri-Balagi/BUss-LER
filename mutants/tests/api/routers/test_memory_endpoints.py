...
async def test_semantic_query(mock_memory_service):
    ...
    mock_memory_service.search_memories.return_value = SearchMemoryResult(
        items=[mock_item], total_count=1
    )

    payload = {"query_text": "Find result"}

    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.post(
            f"/api/v1/twins/{twin_id}/memory/query", json=payload
        )
...
async def test_list_memories(mock_memory_service):
    ...
    mock_memory_service.list_memories.return_value = PaginatedMemories(
        items=[mock_memory], total_count=1, limit=10, offset=0
    )
    ...
    async with AsyncClient(
        ...
        response = await client.get(
            f"/api/v1/twins/{twin_id}/memories?limit=10&offset=0&category=event&include_deleted=false"
        )
